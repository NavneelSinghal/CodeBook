\documentclass[10pt] { extarticle }
\usepackage[T1] { fontenc }
\usepackage { listings }
\usepackage { xcolor }
\usepackage { multicol }
\usepackage[landscape, margin = 1cm, legalpaper] { geometry }
\usepackage[scaled = .8] { beramono }
\renewcommand{\baselinestretch} { 0.7 }
\lstset {
    language = C++, backgroundcolor =\color{black !7},
    % set backgroundcolor basicstyle =\footnotesize\ttfamily,
    % basic font setting columns = fullflexible, showstringspaces = false,
}

\begin { document }

\begin{multicols * } { 3 }

\tableofcontents

    %\section{Bigint} %\lstinputlisting {
    ../ bigint.cpp
}

\section { Data Structures }
\subsection { Vector }
\lstinputlisting { ../ vector.cpp }

\subsection { Deque }
\lstinputlisting { ../ deque.cpp }

\subsection { Priority Queue }
\lstinputlisting { ../ pq.cpp }

\subsection { Pair }
\lstinputlisting { ../ pair.cpp }

\subsection { Fenwick }
\lstinputlisting { ../ fenwick.cpp }

\subsection { Segment Tree }
\lstinputlisting { ../ segtree.cpp }

\subsection { Sparse Table }
\lstinputlisting { ../ sparsetable.cpp }

\subsection { Binary Trie }
\lstinputlisting { ../ binarytrie.cpp }

\subsection { DSU }
\lstinputlisting { ../ dsu.cpp }

\subsection { AVL Tree }
\lstinputlisting { ../ avl.cpp }

\subsection { Matrix }
\lstinputlisting { ../ matrix.cpp }

\section { Graphs }

\subsection { Bridges }
\lstinputlisting { ../ bridges.cpp }
\subsection { Undirected cycle }
\lstinputlisting { ../ undirected_cycle.cpp }
\subsection { Topological sort }
\lstinputlisting { ../ toposort.cpp }
\subsection { Strongly connected components }
\lstinputlisting { ../ scc.cpp }
\subsection {
    Prim's MST}
    \lstinputlisting { ../ prim.cpp }
    \subsection { Minimum cost flow }
    \lstinputlisting { ../ mincostflow.cpp }
    \subsection { LCA }
    \lstinputlisting { ../ lca.cpp }
    \subsection { Floyd Warshall }
    \lstinputlisting { ../ floyd_warshall.cpp }
    \subsection { Dinic }
    \lstinputlisting { ../ dinic.cpp }
    \subsection { Directed cycle }
    \lstinputlisting { ../ directed_cycle.cpp }
    \subsection { Dijkstra }
    \lstinputlisting { ../ dijkstra.cpp }
    \subsection { DFS }
    \lstinputlisting { ../ dfs.cpp }
    \subsection { Bellman Ford }
    \lstinputlisting { ../ bellman_ford.cpp }
    \subsection { Articulation Points }
    \lstinputlisting { ../ articulation_points.cpp }
    \subsection { 2 - SAT }
    \lstinputlisting { ../ 2sat.cpp }
    \subsection { 0 - 1 BFS }
    \lstinputlisting { ../ 01bfs.cpp }

    \section { Strings }

    \subsection { Suffix Array }
    \lstinputlisting { ../ suffixarray.cpp }
    \subsection { KMP }
    \lstinputlisting { ../ kmp.cpp }
    \subsection { Z - function }
    \lstinputlisting { ../ z.cpp }

    \section { GCD }
    \lstinputlisting { ../ gcd.cpp }

    \section { Grid }
    \lstinputlisting { ../ grid.cpp }

    \section { Hash }
    \lstinputlisting { ../ hash.cpp }

    \section { O(1) square root }
    \lstinputlisting { ../ lib.cpp }

    \section { Mint }
    \lstinputlisting { ../ mint.cpp }

    \section { NT }
    \lstinputlisting { ../ nt.cpp }

    \section { NTT }
    \lstinputlisting { ../ ntt.cpp }

    \section { FFT }
    \lstinputlisting{../ fft.cpp}

        %\section{Factorial} %\lstinputlisting{../ fact.cpp}

        %\section{Polynomial} %\lstinputlisting{../ polynomial.cpp}

        %\section{Removable priority queue} %\lstinputlisting {
        ../ removable_pq.cpp
    }

    \section { Template }
    \lstinputlisting { ../ t.cpp }

    \end { multicols* }

    \end { document }
