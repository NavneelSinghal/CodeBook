...
struct u64_omega {
    ...
    [[nodiscard]] constexpr friend T operator+(const T& u, const T& v) {
...
class Conv64 {
   public:
    Conv64() = delete;
    static std::vector<u64> multiply(const std::vector<u64>& p,
                                     const std::vector<u64>& q) {
        if (q.empty() || p.empty()) return {};
        u64 s = 1;
        while (s < p.size() + q.size() - 1) s *= 3;
        std::vector<u64> pp(s), qq(s), res(s);
        std::copy(std::begin(p), std::end(p), std::begin(pp));
        std::copy(std::begin(q), std::end(q), std::begin(qq));
   ...
   private:
    ...
    static void mul_n_omega(R* p, R* q, u64 n, R* to) {
        if (n <= 27) {
            std::fill_n(to, n, R(0));
            for (u64 i = 0; i < n; ++i) {
                for (u64 j = 0; j < n - i; ++j) to[i + j] += p[i] * q[j];
                for (u64 j = n - i; j < n; ++j)
        ...
        for (u64 i = 0; i < r; ++i) {
            ...
            for (u64 j = 0; j < m; ++j) p_i[j].to_conj(), q_i[j].to_conj();
        ...
        for (u64 i = 0; i < r; ++i) {
            ...
            auto* const q_i = q + im;
            for (u64 j = 0; j < m; ++j) {
                to_i[j] += one_minus_omega * to_ni[j] +
                           one_minus_omega2 * q_i[j].conj();
                if (im + m + j < n) {
                    to_i[m + j] +=
                        omega2_minus_omega * (to_ni[j] - q_i[j].conj());
                } else {
                    to[im + m + j - n] +=
                        one_minus_omega2 * (to_ni[j] - q_i[j].conj());
                }
            }
        }
        for (u64 i = 0; i < n; ++i) to[i] *= inv_3;
    }
    ...
    static void mul_n_1(u64* p, u64* q, u64 n, u64* target) {
        ...
        idft(to, m, r);
        for (u64 i = 0; i < n; ++i) pp[i] = to[i] * inv;
        ...
        for (u64 i = 0; i < r; ++i) {
            ...
            for (u64 j = 0; j < m; ++j) {
                to_i[j] += one_minus_omega * pp_i[j] +
                           one_minus_omega2 * pp_i[j].conj();
                if (im + m + j < n) {
                    to_i[m + j] +=
                        omega2_minus_omega * (pp_i[j] - pp_i[j].conj());
                } else {
                    to[im + m + j - n] +=
                        omega2_minus_omega * (pp_i[j] - pp_i[j].conj());
                }
