...
namespace factorize {
    ...
    bool miller_rabin(T n) {
        ...
        static const u64 v[] = {2,      325,     9375,      28178,
    ...
    u64 squfof_iter(u64 &x, u64 &k, u64 &it_max, u32 cutoff_div) {
        ...
        u32 sqrtn = Isqrt(scaledn), cutoff = Isqrt(2 * sqrtn) / cutoff_div,
            q0 = 1, p1 = sqrtn, q1 = scaledn - p1 * p1;
        if (q1 == 0) return __gcd(x, (u64)p1);
        u32 mult = 2 * k, coarse_cutoff = cutoff * mult, p0 = 0, sqrtq = 0, i,
            j;
        for (i = 0; i < it_max; ++i) {
    ...
    u64 SQUFOF(u64 n) {
        constexpr array<u64, 16> multipliers = {
...
int main() {
    ...
    while (q--) {
        ll x;
        cin >> x;
        auto a = factorize::prime_factor(x);
        printf("%zu", a.size());
        for (auto &&i : a) {
            printf(" %lld", i);
        }
