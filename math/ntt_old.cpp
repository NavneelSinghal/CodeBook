...
namespace ntt {
    ...
    template <unsigned Mod>
    vector<Modular<Mod>> operator*(vector<Modular<Mod>> l,
                                   vector<Modular<Mod>> r) {
    ...
    vector<Mint197> operator*(const vector<Mint197>& l,
                              const vector<Mint197>& r) {
        if (l.empty() or r.empty()) return {};
        i32 n = l.size(), m = r.size();
        static constexpr i32 mod0 = 998244353, mod1 = 1300234241,
                             mod2 = 1484783617;
        ...
        for (i32 i = 0; i < n; ++i)
            l0[i] = l[i].v, l1[i] = l[i].v, l2[i] = l[i].v;
        for (i32 j = 0; j < m; ++j)
            r0[j] = r[j].v, r1[j] = r[j].v, r2[j] = r[j].v;
