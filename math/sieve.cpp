...
struct Sieve { 
    ...
    Sieve() {	
        ...
        for (int i = 3; i * i <= SZ; i += 2) if (pri[i]) for (int j = i * i; j <= SZ; j += i << 1) pri[j] = 0;
        for (int i = 1; i <= SZ; ++i) if (pri[i]) pr.push_back(i);
	}
    */
...
struct fast_sieve_func {
    ...
    fast_sieve_func() {
        ...
        for (int i = 2; i <= N; ++i) {
            if (is_prime[i]) primes.push_back(i); // and handle the case of primes by updating func[i]
            for (int j = 0; j < primes.size() && i * primes[j] <= N; ++j) {
                is_prime[i * primes[j]] = false;
...
//#if 0
    ...
    cerr << "Time taken by fast (linear) sieve : " << ((long double)duration.count())/((long double) 1e9) << "s " << endl;

    start = std::chrono::high_resolution_clock::now();
    sieve<N> s2;
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start); 
    cerr << "Time taken by N log log N sieve : " << ((long double)duration.count())/((long double) 1e9) << "s " << endl;
//#endif
...
//#if 0
    ...
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start); 
    cerr << "Time taken by Benq's sieve : " << ((long double)duration.count())/((long double) 1e9) << "s " << endl;
//#endif 
...
//#if 0
    ...
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start); 
    cerr << "Time taken by linear spf sieve : " << ((long double)duration.count())/((long double) 1e9) << "s " << endl;

    start = std::chrono::high_resolution_clock::now();
    spf_sieve<N> s5;
    stop = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start); 
    cerr << "Time taken by N log log N spf sieve : " << ((long double)duration.count())/((long double) 1e9) << "s " << endl;
//#endif
