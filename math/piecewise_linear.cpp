...
struct convex_piecewise_linear {
    ...
    T get_min() const { return minimum; }

    // Equivalent to dp[x] = min(dp[x], ..., dp[x + shift]).
    void expand_negative(T shift) { neg_offset -= shift; }

    // Equivalent to dp[x] = min(dp[x - shift], ..., dp[x]).
    void expand_positive(T shift) { pos_offset += shift; }
...
// https://codeforces.com/contest/372/submission/109441245
void example() {
    int64_t _, M, D;
    cin >> _ >> M >> D;
