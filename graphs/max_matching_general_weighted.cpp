...
class MaximumWeightedMatching {
   ...
   public:
    ...
    vector<InputEdge> get_matching_edges() {
        vector<InputEdge> ans;
        for (int u = 1; u <= N; ++u)
            if (mate[u] > u) {
                cost_t max_c = 0;
                ...
                ans.push_back(InputEdge{u, mate[u], max_c / 2});
            }
        return ans;
    }
   ...
   private:
    ...
    void reset_all() {
        ...
        link[0].from = 0;
        for (int v = 1; v <= N;
             ++v) {  // should be optimized for sparse graphs.
            if (label[v] == kOuter)
        ...
        for (int d = 0; d < 2; ++d) {
            for (int bv = surface[x]; bv != lca;) {
                ...
                push_outer_and_fix_potentials(
                    bmv, fix_blossom_potential<kInner>(bmv));
    ...
    void expand(int bid) {
        ...
        for (int b = old_base;; b = node[b].link[d].b) {
            ...
            if (b == root) break;
            push_outer_and_fix_potentials(
                nb, fix_blossom_potential<kInner>(b = nb));
        }
    }
