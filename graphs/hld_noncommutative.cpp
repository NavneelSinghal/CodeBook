...
struct NonCommutativeHLD {
    ...
    void update_path(int u, int v, update_t val) {
        ...
        while (u != -1 && is_anc(l, u)) {
            ...
            int qr = N - (max(in_time[start[u]], in_time[l] + vals_in_edges));
            tree_forward.update(N - qr, N - ql, val);
            tree_reverse.update(ql, qr, val);
            u = par[start[u]];
        }
        while (v != -1 && is_anc(l, v)) {
            ...
            int qr = in_time[v] + 1;
            tree_forward.update(ql, qr, val);
            tree_reverse.update(N - qr, N - ql, val);
            v = par[start[v]];
        }
    }
    node_t query_path(int u, int v) {
        ...
        while (u != -1 && is_anc(l, u)) {
            int ql = N - (in_time[u] + 1);
            int qr = N - (max(in_time[start[u]], in_time[l] + vals_in_edges));
            res_l = combine(res_l, tree_reverse.query(ql, qr));
            u = par[start[u]];
        }
        while (v != -1 && is_anc(l, v)) {
            int ql = max(in_time[start[v]],
                         in_time[l] + 1);  // to avoid overcounting
            int qr = in_time[v] + 1;
            res_r = combine(tree_forward.query(ql, qr), res_r);
            v = par[start[v]];
        }
        // NOTE: this will fail if you put an assert in the combine function,
        // since the indices belong to different segment trees
        return combine(res_l, res_r);
    }
    node_t query_vertex(int u) {
        return tree_forward.query(in_time[u], in_time[u] + 1);
    }
    void update_vertex(int u, update_t val) {
        int l = in_time[u];
        int r = l + 1;
        tree_forward.update(l, r, val);
        tree_reverse.update(N - r, N - l, val);
    }
    ...
    node_t query_subtree(int u) {
        return tree_forward.query(in_time[u] + vals_in_edges,
                                  in_time[u] + sz[u]);
    }
    void dfs_sz(int u) {
        sz[u] = 1;
