...
struct QueueAggregate {
    ...
    void pop_front() {
        if (values[0].empty()) {
            std::ptrdiff_t smaller = 0;
            values[0] =
                std::vector<T>(values[1].rbegin() + smaller, values[1].rend());
            values[1] = std::vector<T>{};
            aggregates[1].clear(), aggregates[0].clear();
...
int main() {
    ...
    for (i32 _test = 1; _test <= _tests; ++_test) {
        ...
        u32 q;
        cin >> q;
        QueueAggregate d(op, std::pair<mint, mint>{1, 0});
        while (q--) {
            u32 t;
            cin >> t;
            if (t == 0) {
