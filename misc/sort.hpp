...
void counting_sort_impl(It begin, It end, OutIt out_begin,
                        ExtractKey&& extract_key) {
    counting_sort_impl<std::uint64_t>(begin, end, out_begin,
                                      extract_key);
}
...
struct SizedRadixSorter<2> {
    ...
    static bool sort_inline(It begin, It end, OutIt out_begin,
        ...
        for (OutIt it = out_begin; it != out_end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 8;
...
struct SizedRadixSorter<4> {
    ...
    static bool sort_inline(It begin, It end, OutIt out_begin,
        ...
        for (It it = begin; it != end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 16;
            out_begin[counts2[key]++] = std::move(*it);
        }
        for (OutIt it = out_begin; it != out_end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 24;
...
struct SizedRadixSorter<8> {
    ...
    static bool sort_inline(It begin, It end, OutIt out_begin,
        ...
        for (It it = begin; it != end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 32;
            out_begin[counts4[key]++] = std::move(*it);
        }
        for (OutIt it = out_begin; it != out_end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 40;
            begin[counts5[key]++] = std::move(*it);
        }
        for (It it = begin; it != end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 48;
            out_begin[counts6[key]++] = std::move(*it);
        }
        for (OutIt it = out_begin; it != out_end; ++it) {
            std::uint8_t key =
                to_unsigned_or_bool(extract_key(*it)) >> 56;
...
// 1. they serve as illustration for how to implement the
// to_radix_sort_key function
// 2. they help produce better error messages. with these overloads you
// get the
//    error message "no matching function for call to
//    to_radix_sort(your_type)" without these examples, you'd get the
//    error message "to_radix_sort_key was not declared in this scope"
//    which is a much less useful error message
...
struct ListElementSubKey
    ...
    static decltype(auto) sub_key(U&& value, void* sort_data) {
        ...
        const T& list = CurrentSubKey::sub_key(
            value, list_sort_data->next_sort_data);
...
template <std::ptrdiff_t StdSortThreshold, typename It,
          typename ExtractKey>
...
struct UnsignedInplaceSorter {
    ...
    static void sort(It begin, It end, std::ptrdiff_t num_elements,
        ...
        if (num_elements < AmericanFlagSortThreshold)
            american_flag_sort(begin, end, extract_key, next_sort,
                               sort_data);
        else
            ska_byte_sort(begin, end, extract_key, next_sort,
                          sort_data);
    }

    template <typename It, typename ExtractKey>
    static void american_flag_sort(
        ...
        if (num_partitions > 1) {
            ...
            uint8_t* last_block =
                remaining_partitions + num_partitions - 1;
            It it = begin;
            ...
            for (uint8_t *it = remaining_partitions,
                ...
                if (!StdSortIfLessThanThreshold<StdSortThreshold>(
                    ...
                    UnsignedInplaceSorter<
                        StdSortThreshold, AmericanFlagSortThreshold,
...
struct UnsignedInplaceSorter<StdSortThreshold,
                             AmericanFlagSortThreshold, CurrentSubKey,
                             NumBytes, NumBytes> {
    template <typename It, typename ExtractKey>
...
size_t CommonPrefix(It begin, It end, size_t start_index,
    ...
    for (++begin; begin != end; ++begin) {
        ...
        for (size_t i = start_index + 1; i < largest_match; ++i) {
            if (element_key(largest_match_list[i]) !=
    ...
    static void sort(It begin, It end, ExtractKey& extract_key,
        ...
        if (sort_data->next_sort &&
            !StdSortIfLessThanThreshold<StdSortThreshold>(
                ...
                extract_key)) {
            sort_data->next_sort(begin, end_of_shorter_ones,
        ...
        if (!StdSortIfLessThanThreshold<StdSortThreshold>(
            ...
            InplaceSorter<StdSortThreshold, AmericanFlagSortThreshold,
                          ElementSubKey>::sort(end_of_shorter_ones, end,
                                               num_elements,
                                               extract_key,
                                               sort_next_element,
                                               sort_data);
...
template <std::ptrdiff_t StdSortThreshold,
          std::ptrdiff_t AmericanFlagSortThreshold,
          typename CurrentSubKey>
struct InplaceSorter<StdSortThreshold, AmericanFlagSortThreshold,
                     CurrentSubKey, bool> {
    template <typename It, typename ExtractKey>
    static void sort(It begin, It end, std::ptrdiff_t,
                     ExtractKey& extract_key,
...
template <std::ptrdiff_t StdSortThreshold,
          ...
          typename CurrentSubKey, typename SubKeyType>
struct FallbackInplaceSorter<
    StdSortThreshold, AmericanFlagSortThreshold, CurrentSubKey,
    SubKeyType,
...
template <std::ptrdiff_t StdSortThreshold,
          std::ptrdiff_t AmericanFlagSortThreshold>
struct SortStarter<StdSortThreshold, AmericanFlagSortThreshold,
                   SubKey<void>> {
    template <typename It, typename ExtractKey>
...
bool ska_sort_copy(It begin, It end, OutIt buffer_begin) {
    return ska_sort_copy(begin, end, buffer_begin,
