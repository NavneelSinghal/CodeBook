...
void example() {
    ...
    while (F.back() < N) F.push_back(F[F.size() - 2] + F.back());
    ...
    cout << (y_combinator([&](auto solve, int root, int n) -> bool {
        ...
        if (!binary_search(F.begin(), F.end(), n)) return false;
        ...
        y_combinator([&](auto dfs, int node, int parent) -> void {
            ...
            if (subtree_size[node] == F[f - 2] ||
                subtree_size[node] == F[f - 1]) {
                split = node;
                par = parent;
            }
        })(root, -1);
        ...
        adj[split].erase(remove(adj[split].begin(), adj[split].end(), par),
                         adj[split].end());
        adj[par].erase(remove(adj[par].begin(), adj[par].end(), split),
                       adj[par].end());
